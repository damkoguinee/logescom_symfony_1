...
class LivraisonController extends AbstractController
{
    ...
    public function index(LieuxVentes $lieu_vente, Request $request, UserRepository $userRep, AuthorizationCheckerInterface $authorizationChecker, LivraisonRepository $livraisonRep, EntrepriseRepository $entrepriseRep): Response
    {
        ...
        if ($request->get("id_client_search")){
           $livraisons = $livraisonRep->findLivraisonByLieuBySearchPaginated($lieu_vente, $search, $date1, $date2, $pageEncours, 50);
        }elseif ($request->get("id_personnel")){
           $livraisons = $livraisonRep->findLivraisonByLieuByPersonnelPaginated($lieu_vente, $request->get("id_personnel"), $date1, $date2, $pageEncours, 50);
        }else{
            if (!$authorizationChecker->isGranted('ROLE_GESTIONNAIRE')) {
               $livraisons = $livraisonRep->findLivraisonByLieuByPersonnelPaginated($lieu_vente, $this->getUser(), $date1, $date2, $pageEncours, 50);
            }else{
               $livraisons = $livraisonRep->findLivraisonByLieuPaginated($lieu_vente, $date1, $date2, $pageEncours, 50);
            }
        }
    ...
    public function facturationsNonLivres(LieuxVentes $lieu_vente, Request $request, UserRepository $userRep, AuthorizationCheckerInterface $authorizationChecker, FacturationRepository $facturationRep, EntrepriseRepository $entrepriseRep): Response
    {
        ...
        $pageEncours = $request->get('pageEncours', 1);
        if ($request->get("id_client_search")){
            $facturations = $facturationRep->findFacturationByLieuBySearchPaginated($lieu_vente, $search, $date1, $date2, $pageEncours, 50);
        }elseif ($request->get("id_personnel")){
            $facturations = $facturationRep->findFacturationByLieuByPersonnelPaginated($lieu_vente, $request->get("id_personnel"), $date1, $date2, $pageEncours, 50);
        }elseif ($request->get("numero_facture")){
            $facturations = $facturationRep->findFacturationByNumeroPaginated($request->get("numero_facture"), $pageEncours, 50);
        }else{
            if (!$authorizationChecker->isGranted('ROLE_GESTIONNAIRE')) {
    ...
    public function edit(Facturation $facturation, LieuxVentes $lieu_vente, Request $request,  StockRepository $stockRep, ProductsRepository $productRep, ListeStockRepository $listeStockRep,  CommandeProductRepository $commandeProdRep, EntityManagerInterface $em): Response
    {
