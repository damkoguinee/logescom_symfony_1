...
class BilanController extends AbstractController
{
    ...
    public function etatCaisse(LieuxVentes $lieu_vente, CaisseRepository $caisseRep, DeviseRepository $deviseRep, MouvementCaisseRepository $mouvementRep, UserRepository $userRep, EntrepriseRepository $entrepriseRep): Response
    {
        $caisses = $caisseRep->findCaisseByLieu($lieu_vente);
        $devises = $deviseRep->findAll();
        $solde_caisses = $mouvementRep->soldeCaisseParLieu($lieu_vente, $devises, $caisses);
        $caisses_lieu = [];
        foreach ($solde_caisses as $solde) {
            foreach ($caisses as $caisse) {
                if ($solde['id_caisse'] == $caisse->getId()) {
